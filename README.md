...
npm run scenario:A       # or B, C, D, E, F, G, H, I, J, L individually; `npm run scenarios > evidence/mirror-scenarios-A-J-$(date +%F)-create2-factory.log 2>&1` refreshes the evidence log
```

## Phase 2: relay + beacon (agent interface, docs/RELAY.md)
```
npm run deploy:local -- --keep                                   # leave anvil up; deployments/local.json
npm run beacon:build -- --deployment deployments/local.json --origin http://127.0.0.1:18751 --out beacon/public   # with no --origin: the canonical https://relay-zero.mkyang.ai
RELAY_SPONSOR_KEY=0x<key> RELAY_BEACON_DIR=beacon/public npm run relay   # sponsored GET intents on http://127.0.0.1:18751, and it serves README.txt, the snippets, CONSTITUTION.md and the dashboard
npm run beacon:validate -- --deployment deployments/local.json --out beacon/public   # README <= 44 lines, addresses have code, every advertised URL answers 200 from the origin
npm run e2e:relay                                                # cold-start path through README + relay only (T1 snippets, T0 pass)
npm run e2e:entry-point                                          # the relay is the beacon; a challenged origin fails validate (evidence/phase5/entry-point/)
```
## Phase 3: Base Sepolia (chain 84532; docs/TESTNET_PLAN.md, docs/MAINNET_PLAN.md)
Deployment `deployments/base-sepolia.json` (18 contracts verified on Basescan, inputs under `deployments/verification-base-sepolia/`). Canonical entry point `https://relay-zero.mkyang.ai` (launchd + cloudflared on the mini, runtime `~/srv/zero-one-dao`): the relay serves README.txt, the snippets, CONSTITUTION.md and the dashboard next to its live endpoints, because a plain GET is the whole onboarding and a bot challenge would end it (decision.md 2026-09-10). Mirror `https://zero-one-beacon.vercel.app`: same files, /relay and /me rewritten to the relay, named in one README line. Evidence under `evidence/testnet/`.
```
...
tsx relay/cold-start.ts --beacon https://relay-zero.mkyang.ai --host mini|main         # README + relay only (T1 and T0); --beacon <mirror> exercises the mirror
tsx relay/corner-cases-relay.ts --beacon https://relay-zero.mkyang.ai                 # relay / adapter / beacon rows
npm run beacon:build -- --deployment deployments/base-sepolia.json --out <dir> && npm run beacon:validate -- --deployment deployments/base-sepolia.json --out <dir>   # canonical; the relay runs with RELAY_BEACON_DIR=<dir>
npm run beacon:build -- --deployment deployments/base-sepolia.json --origin https://zero-one-beacon.vercel.app --out beacon/public-base-sepolia && npm run beacon:validate -- --deployment deployments/base-sepolia.json --out beacon/public-base-sepolia --origin mirror && tsx beacon/scripts/vercel.ts --out beacon/public-base-sepolia   # mirror
```
