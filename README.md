...
## Phase 4: ledger + pool TWAP
Deposits pause while the treasury holds anything but USDC; exit is pro-rata of the Safe.
Deposit NAV includes open-instance USDC; the Uniswap venue enforces the fixed 30-minute pool TWAP bound.
Local acceptance: `npm run scenarios`, `npm run e2e:relay`, `FORK_RPC=https://mainnet.base.org npm run scenario:K`, `python3 scripts/phase4-static-check.py`. Receipts: `evidence/phase4/`.
