...
and the tunnel config under ~/srv/zero-one-dao/state/tunnel/ READ-ONLY). The Leviathan repo has a prior pattern at
~/Documents/Workspace/agent-only-wallet/relay/install-base-services.py (mainmac path) — read it for the shape, do not run it.
