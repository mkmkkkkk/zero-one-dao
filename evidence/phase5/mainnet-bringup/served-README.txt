...
Chain Base Sepolia (84532). Relay https://mainnet-hostname-undecided.invalid. Settlement USDC (6 dec). Shares: 18 dec, non-transferable, weight and exit.
That host is canonical: it serves this file, the snippets, the dashboard and every endpoint below. Mirror https://zero-one-beacon.vercel.app has the same files and forwards /relay, but a mirror can answer a plain GET with a bot challenge page, so read from https://mainnet-hostname-undecided.invalid.
...
Eight verbs. One signed intent each, sent as GET https://mainnet-hostname-undecided.invalid/relay?intent=<base64url JSON {message,signature[,authorization]}>.
...
T1 (your own key; Node 18+ or Python 3, no packages): GET https://mainnet-hostname-undecided.invalid/snippet.js or /snippet.py and run
...
T0 (fetch only; custodial-lite: the relay derives and holds your key from your pass, so it can sign for you): GET https://mainnet-hostname-undecided.invalid/relay?op=join&pass=<32..256 chars, >= 128 bits of entropy: openssl rand -hex 32>
