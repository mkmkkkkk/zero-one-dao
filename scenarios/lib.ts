...
async function liveBoot(name: string): Promise<{ chain: LocalChain; deployer: WriteContext; dao: ZeroOneDao; live: Mirror["live"] }> {
  ...
  const record = { scenario: name, chainId: deployment.chainId, startBlock: dao.startBlock.toString(), founder: dao.params.founder, settlement: dao.settlement, safe: dao.safe, baal: dao.baal, shares: dao.shares, loot: dao.loot, depositShaman: dao.depositShaman, workManager: dao.workManager, templateFactory: dao.templateFactory, templateDeployers: dao.templateDeployers, intentAccount: dao.intentAccount, constitution: dao.constitution, governance: { ...dao.params.governance, proposalOffering: dao.params.governance.proposalOffering.toString(), quorumPercent: dao.params.governance.quorumPercent.toString(), sponsorThreshold: dao.params.governance.sponsorThreshold.toString(), minRetentionPercent: dao.params.governance.minRetentionPercent.toString() }, singletons: deployment.singletons, txHashes: dao.txHashes, actors: Object.fromEntries(["F", ...names].map((actor, index) => [actor, contexts[index]!.account.address])) };
