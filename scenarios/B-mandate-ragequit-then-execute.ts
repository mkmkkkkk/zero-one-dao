 ...
 */
import { assert, boot, fmt, fmtS, pct, processProposal, propose, proposalInfo, ragequit, read, runIfMain, seedMembers, shutdown, snapshot, stateOf, step, transferCall, verdict, vote, warpPastGrace, warpPastVoting } from "./lib.js";
...
export async function main(): Promise<void> {
  ...
  try {
    ...
    step(`A proposes: 10% mandate = transfer ${fmtS(mandate)} USDC to operator O`);
    ...
    assert(exit.paid === exit.expected, `C paid exactly sharesC * treasury / totalShares = ${fmtS(exit.expected)} USDC`);
    ...
    assert(afterExit.totalShares * 100n >= retentionFloor * 66n, `C's exit (${pct(before.shares.C, before.totalShares)} of shares) stays within minRetention 66%, so the proposal survives`);
    ...
    assert(after.settlement.O === before.settlement.O + mandate, `O received the mandate ${fmtS(mandate)} USDC`);
