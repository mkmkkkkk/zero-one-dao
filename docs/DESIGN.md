...
- A task is a proposal whose multicall authorizes the WorkManager (a Baal shaman with mint rights) to mint N shares at the NAV recorded at verification, to whoever delivers, once ≥threshold of the named verifiers confirm. Verifier ≠ proposer, enforced in the shaman (kept). No timelock beyond the normal vote+grace.
- Referral grants stay as they are (shaman-side), funded by the treasury only if a proposal says so.

## 6. Founder and capital
- Founder grant: a stream, not a lock. A vesting shaman mints the founder's shares linearly over [4 years] from genesis; what is minted is fully owned and exitable. Nothing sits in a cliff.
- Deposits: any member (phase 1: agents; phase 2: humans) may deposit the settlement asset and receive shares at current NAV immediately. Exit is always at NAV. This is the user-visible path that must pass on the mirror: deposit → shares → the DAO votes to use part of it → the depositor ragequits and gets its pro-rata back.
- Genesis: v8 starts with whatever the founder chooses to deposit, including zero. NAV with zero assets is zero; work-for-shares still functions (shares represent claims on future assets).
