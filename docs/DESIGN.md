...
- Every template is owned by the Safe: a later passed proposal can call topUp(amount), amend(params), stop() (returns all funds to the Safe), or migrate(newContract) (moves funds to a new voted contract). The proposer cannot do any of these alone. This is how a project whose budget proved too small gets more by a second vote, and how a bad strategy is stopped by a vote.
