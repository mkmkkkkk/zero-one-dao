...
Proposals and votes
- Vote after voting ends; vote twice; sponsor with < 1 share; process before grace ends; process twice; proposal whose multicall reverts (actionFailed=true: funds untouched); proposal that funds a contract with a bug that traps funds (stop() must still work; if the template cannot return, that is a template bug → fix).
